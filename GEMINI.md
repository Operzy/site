...
- `100dvh` height. Full-bleed dimensional background with an organic image texture (Unsplash matching preset's `imageMood`), deep tech overlay gradients, floating animated CSS orbs (with pulse keyframes), and layered "data stream" lines to create a rich, sexy environment.
- **Layout:** Side-by-side alignment block. Text content pushed to the left (`lg:w-[55%]`), and the main product/service mockup container on the right (`lg:w-[45%]`). The mockup should have a 3D perspective tilt (`rotate-y-[-5deg]`) that squares up seamlessly on hover.
- **Typography:** Large scale contrast following the preset's hero line pattern. First part in bold sans heading font. Second part in massive serif italic drama font (3-5x size difference).
- **Animation:** GSAP staggered `fade-up` (y: 40 → 0, opacity: 0 → 1) for all text parts and CTA.
- CTA button below the headline, using the accent color.

### C. CORE FRAMEWORKS — "Inside the System"
A 4-card grid mapping out the curriculum, methodology, or core architecture.
- **Interactions:** Cards must be highly interactive. On hover: elevate (`-translate-y-2`), drop a colorful shadow, expand a gradient underline beneath the heading, and reveal a sleek tracking target / crosshair indicator with a subtle inner glowing background layer.

### D. FEATURES / BONUSES — "Interactive Functional Artifacts"
Three cards derived from the user's 3 value propositions positioned as included toolkits or "Bonuses". These must feel like **functional software micro-UIs**. Each card has a strict `flex flex-col` design where the very bottom footer line contains a "VALUE: $XXX" tag to actively anchor the bonus worth.
...
**Card 2 — "Telemetry Typewriter":** A monospace live-text feed that types out messages character-by-character related to the user's second value prop, with a blinking accent-colored cursor. Include a "Live Feed" label with a pulsing dot. Hide the container's native scrollbar (`scrollbar-width: none`).
...
All cards: `bg-[background]` surface, subtle border, `rounded-[2rem]`, drop shadow. Each card has a heading (sans bold), brief descriptor, and bottom value bar.
